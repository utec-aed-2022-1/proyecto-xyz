...
auto sha256(std::vector<bool> bits) -> std::array<uint32_t, 8>
{
    ...
    while (bits_i != bits.size())
    {
        ...
        {
            uint32_t s0 = right_rotate(w.at(i - 15), 7) ^ right_rotate(w.at(i - 15), 18)
                          ^ (w.at(i - 15) >> 3);

            uint32_t s1 = right_rotate(w.at(i - 2), 17) ^ right_rotate(w.at(i - 2), 19)
                          ^ (w.at(i - 2) >> 10);
...
auto sha256(uint8_t const* octets, size_t n_octets) -> std::array<uint32_t, 8>
{
    ...
    size_t remaining_octets = n_octets;
    ...
    auto process_512 = [&](auto const& w_given) {
        ...
        {
            uint32_t s0 =
                right_rotate(w(i - 15), 7) ^ right_rotate(w(i - 15), 18) ^ (w(i - 15) >> 3);

            uint32_t s1 =
                right_rotate(w(i - 2), 17) ^ right_rotate(w(i - 2), 19) ^ (w(i - 2) >> 10);
    ...
    while (remaining_octets > 0)
    {
        if (512 <= remaining_octets * 8)
        {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
            auto w_given = reinterpret_cast<uint32_t const*>(octets);
            process_512([&](size_t index) { return w_given[index]; });

            remaining_octets -= 512 / 8;
        }
        // We can fit the remaining octets and the size in the same 512 block
        else if (remaining_octets * 8 + 64 <= 512)
        {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
            auto octs = reinterpret_cast<uint32_t const*>(octets);

            process_512([&](size_t index) {
                if (index < remaining_octets / 4)
                {
                    return octs[index];
                }
                else if (index == remaining_octets / 4)
                {
                ...
                {
                    return static_cast<uint32_t>((n_octets * 8) >> 32);
                }
                else
                {
                    return static_cast<uint32_t>(n_octets * 8);
                }
            });
        ...
        else
        {
            ...
            process_512([&](size_t index) {
                if (index < remaining_octets / (32 / 8))
                {
                    return octs[index];
                }
                else
...
auto to_hex_string(uint32_t n) -> std::string
{
    ...
    return to_hex_string(bs(n >> 28)) + to_hex_string(bs(n >> 24)) + to_hex_string(bs(n >> 20))
           + to_hex_string(bs(n >> 16)) + to_hex_string(bs(n >> 12)) + to_hex_string(bs(n >> 8))
